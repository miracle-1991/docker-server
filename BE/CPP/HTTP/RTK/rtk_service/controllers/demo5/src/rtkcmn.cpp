...
const solopt_t solopt_default={ /* defaults solution output options */
    SOLF_LLH,TIMES_GPST,0,3,    /* posf,times,timef,timeu */
...
{
    if (obs->data != NULL) free(obs->data); obs->data=NULL; obs->n=obs->nmax=0;
}
...
{
    if (opt&0x01) { if (nav->eph != NULL) free(nav->eph ); nav->eph =NULL; nav->n =nav->nmax =0;}
    if (opt&0x02) { if (nav->geph != NULL) free(nav->geph); nav->geph=NULL; nav->ng=nav->ngmax=0;}
    if (opt&0x04) { if (nav->seph != NULL) free(nav->seph); nav->seph=NULL; nav->ns=nav->nsmax=0;}
    if (opt&0x08) { if (nav->peph != NULL) free(nav->peph); nav->peph=NULL; nav->ne=nav->nemax=0;}
    if (opt&0x10) { if (nav->pclk != NULL) free(nav->pclk); nav->pclk=NULL; nav->nc=nav->ncmax=0;}
    if (opt&0x20) { if (nav->alm != NULL) free(nav->alm ); nav->alm =NULL; nav->na=nav->namax=0;}
    if (opt&0x40) { if (nav->tec != NULL) free(nav->tec ); nav->tec =NULL; nav->nt=nav->ntmax=0;}
}
...
static lock_t lock_trace;       /* rtklock for trace */
