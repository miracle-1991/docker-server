...
func ExistSchedule(cs_flight_iata string, dep_time_ts, arr_time_ts uint32) (bool, error) {
	var item Schedule
	err := common.DB.Select("id").Where("cs_flight_iata = ? AND dep_time_ts = ? AND arr_time_ts = ? AND deleted_on = ? ", cs_flight_iata, dep_time_ts, arr_time_ts, 0).First(&item).Error
